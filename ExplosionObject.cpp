...
bool ExplosionObject::LoadImg(std::string path, SDL_Renderer* screen)
{
    ...
    if (ret)
    {
        ...
        frame_height_ = rect_.h/1;
    }
    return ret;
}
void ExplosionObject::set_clips()
{
    if (frame_width_ > 0 && frame_height_ > 0) //nếu loadimg thành công
    {
        frame_clip_[0].x = 0;
        frame_clip_[0].y = 0;
        ...
        frame_clip_[3].h = frame_height_;  // vd: frame 4 sẽ ở vị trí trong ảnh sao cho nó cắt đc trọn vẹn frame thứ 4 trong ảnh
